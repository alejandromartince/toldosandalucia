        ...
        <div className="contenedorTitulosFotosBeneficios">
          {datos.map((item, index) => (
            <div className="tituloContainer" key={index}>
              <h3
                className={`titulo ${activo === index ? "activo" : ""} ${
                  activo !== null && activo !== index ? "invisible" : ""
                }`}
        ...
        {datos.map((item, index) => {
          ...
          return (
            <div
              key={index}
              onClick={() => manejarClick(index)}
              className={`imagen-item ${mostrar ? "visible" : "oculto"} ${
                esActivo ? "expandida" : ""
              }`}
            >
              {dispositivo === "movil" && <h3>{item.titulo}</h3>}
              {esActivo ?  <BiCollapse size={24} /> : <FaExpand size={24} />}
